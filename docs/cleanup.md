# Accounting Cleanup Flow

This document outlines the end-to-end architecture and data flow for the accounting cleanup process, detailing how raw CSV/Excel bank statements uploaded by users are processed, mapped, and semantically enriched by the TAP (Toro Agent Protocol) Agent Network.
...
2. **TAP Cleanup Agent**: Uses LLMs to infer the structure of the CSV and standardize column names.
3. **Cleanup Worker**: Persists the standardized raw rows into the staging database.
...
- **Process:**
  ...
  4. The envelope is broadcasted to the NATS JetStream topic: `tasks.accounting.cleanup.0`.
  5. The API returns `202 Accepted` to the client, along with a JSON payload containing the `session_id` and `status` ("PROCESSING").

### 2. AI Column Mapping (`TAP Cleanup AI Agent`)
**File:** `tap/agents/cleanup/agent.go`

- **Initialization:** Registers its capabilities (`accounting.cleanup`) with the Almanac registry on startup.
- **Trigger:** Listens to `tasks.accounting.cleanup.>` CFPs on NATS JetStream.
- **Process:**
...
- **Process:**
  1. Unmarshals the `INFORM` envelope containing the `RawRow` structured data from the Cleanup Agent.
...
- **Trigger:** Listens directly to the `proof.accounting.cleanup.columns` (`INFORM` proof) on NATS JetStream. *(Pipeline execution: it is informed by the Cleanup Agent's completion rather than responding to a new `CFP`.)*
- **Process:**
  1. **Sync Wait:** Polls the database (`GetPendingSessionRows`) for up to 10 seconds to ensure the `Cleanup Worker` has finished inserting the staged rows.
