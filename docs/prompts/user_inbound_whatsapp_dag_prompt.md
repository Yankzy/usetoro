TAP User Inbound WhatsApp DAG Generation Prompt
...
````
You are an expert system configurator for the Toro Autonomous Semantic Engine (ASE). Generate a YAML file representing the `user_inbound_whatsapp` DAG for a newly onboarded user.
...
3. The DAG must include at least these three base nodes:
   - `extract_intent` (kind: `classifier`): Analyzes the incoming **WhatsApp message** to extract the user's intent.
...
4. The `prompts` section must contain the specific instructions for each classifier.
   - For the `intent_specialist` prompt, you MUST explicitly mention that the message is a **WhatsApp message**. Keep in mind that WhatsApp messages are often short, informal, and may contain voice note transcriptions, emojis, or rapid multi-part messages.
...
prompts:
  intent_specialist:
    - |
      You are an Inbound WhatsApp Triage node for Tenant: [TENANT_ID]. 
      Read the incoming WhatsApp message and determine the user's intent. Expect informal language, emojis, or short sentences.
      
      Custom Rules: [SPECIFIC_RULES]
      
      Select from the following intents:
      1. PROVIDE_RECEIPT: The user has uploaded an image of a receipt/invoice.
      2. CLARIFICATION: The user is answering a question about a transaction.
      3. APPROVAL: The user gave a quick "yes", "thumbs up", or approval.
        ...
        [
          {
            ...
            "value": {
              ...
              "property": "intent",
              "candidates": {
              "1": { "value": "APPROVAL", "confidence": 0.99, "reasoning": "User sent a thumbs up emoji." },
