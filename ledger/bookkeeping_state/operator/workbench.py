...
class BookkeepingWorkbench:
    ...
    _default_repository_factory: ClassVar[Any] = None
    _default_dag_classifier_factory: ClassVar[Any] = None
    ...
    def __init__(
        ...
        *,
        ...
        self.active_challenge: Any = None
        self.active_temporal_challenge: Any = None
        ...
        elif getattr(self.__class__, "_default_repository_factory", None) is not None:
            factory = getattr(self.__class__, "_default_repository_factory")
            self.repository, self.company_id = factory()
            self._configure_services()
            self.hydrator = BookkeepingHydrator(repository=self.repository)
            self.session_counter += 1
    ...
    def _update_repository_snapshot(self, new_snap: BookkeepingSnapshot) -> None:
        ...
        if hasattr(self.repository, "update_snapshot"):
            getattr(self.repository, "update_snapshot")(new_snap)
        elif hasattr(self.repository, "set_snapshot"):
            getattr(self.repository, "set_snapshot")(new_snap)
        else:
            raise NotImplementedError(
                f"Repository {type(self.repository).__name__} does not support in-place snapshot update"
            )
    ...
    def _get_dag_classifier(self) -> AseClassifier:
        ...
        factory = getattr(self.__class__, "_default_dag_classifier_factory", None)
        if factory is not None:
            return factory()
        raise ValueError("No DAG classifier configured for BookkeepingWorkbench")
    ...
    def _commit_source_writeset(
        ...
        return {
            ...
            "message": f"Successfully committed {desc} (persistence revision {new_p}).",
        }

    def _execute_mutation(
        self,
        command_factory: Callable[[BookkeepingState], Any],
        desc: str,
    ) -> dict[str, Any]:
        """
        Execute an internal state mutation via TransitionEngine against a isolated mutation state.
        Fails closed: inspection state is closed immediately before mutation.
        """
        self.close_state()
        prev_p: int | None = None
    ...
    _apply_transition_command = _execute_mutation
    ...
    def set_auto_reconcile_unique_inferred_allocation(
        ...
        *,
        ...
        self._update_repository_snapshot(new_snap)
        scenario_clock = self.clock_time
