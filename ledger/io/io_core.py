"""
This module serves as a core component of the Ledger framework, providing an optimized interface and
...
2. **Validation Utilities**:
   ...
   - Ensures consistency with Ledger configurations, such as handling timezone-awareness and closing dates.
...
- Optional PDF generation using settings from Ledger.

Notes:
------
- Ensure `LEDGER_PDF_SUPPORT_ENABLED` is properly configured to enable PDF output in financial reports.
...
from ledger.exceptions import InvalidDateInputError, TransactionNotInBalanceError
from ledger.io import roles as roles_module, CREDIT, DEBIT
from ledger.io.io_context import IODigestContextManager
from ledger.io.io_middleware import (
...
from ledger.io.ratios import FinancialRatioManager
from ledger.models.utils import lazy_loader
from ledger.settings import LEDGER_PDF_SUPPORT_ENABLED
...
def diff_tx_data(tx_data: list, raise_exception: bool = True):
    ...
    if not is_valid and abs(diff) > settings.LEDGER_TRANSACTION_MAX_TOLERANCE:
        if raise_exception:
            raise TransactionNotInBalanceError(
                f'Invalid tx data. Credits and debits must match. Currently cr: {credits}, db {debits}.'
                f'Max Tolerance {settings.LEDGER_TRANSACTION_MAX_TOLERANCE}'
            )
...
def check_tx_balance(tx_data: list, perform_correction: bool = False) -> bool:
    ...
    if tx_data:
        ...
        if not perform_correction and abs(diff) > settings.LEDGER_TRANSACTION_MAX_TOLERANCE:
            return False
        ...
        while not is_valid:
            ...
            if len(txs_candidates) > 0:
                ...
                if any([diff > 0 and tx_type_choice == DEBIT,
                    ...
                    if IS_TX_MODEL:
                        tx.amount += settings.LEDGER_TRANSACTION_CORRECTION
                    else:
                        tx['amount'] += settings.LEDGER_TRANSACTION_CORRECTION
...
class IODatabaseMixIn:
    ...
    def database_digest(self,
        ...
        USE_CLOSING_ENTRIES = settings.LEDGER_USE_CLOSING_ENTRIES
...
class IOReportMixIn:
    ...
    def get_balance_sheet_statement(self,
        ...
        `LEDGER_PDF_SUPPORT_ENABLED` flag. If the flag is not enabled, the
        ...
        **kwargs
        ...
        """
        if not LEDGER_PDF_SUPPORT_ENABLED:
            raise IOValidationError(
    ...
    def get_cash_flow_statement(self,
        ...
        ------
        IOValidationError
            If PDF support is not enabled in the system's Ledger configuration.
        """
        if not LEDGER_PDF_SUPPORT_ENABLED:
            raise IOValidationError(
                message=_('PDF support not enabled. Install PDF support from Pipfile.')
