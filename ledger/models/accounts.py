...
AccountModel
------------
The AccountModel is a fundamental component of the Ledger system, responsible for categorizing and organizing financial transactions related to an entity's assets, liabilities, and equity.
...
Users have the flexibility to adopt a chart of accounts that best suits their EntityModel. Ledger provides a default Chart of Accounts when creating a new EntityModel, which can be customized as needed.

Account Roles
-------------

All AccountModels must be assigned a role from the `ACCOUNT_ROLES` function in io.roles.
...
from ledger.settings import LEDGER_ACCOUNT_CODE_GENERATE, LEDGER_ACCOUNT_CODE_USE_PREFIX
...
class AccountModelAbstract(MP_Node, CreateUpdateMixIn):
    ...
    coa_model = models.ForeignKey('ledger.ChartOfAccountModel',
    ...
    def generate_next_coa_code(self):
        """
        Assigns the next available code from chart_of_accounts_with_roles.json.
        Sets name and role from the JSON as well.
        Raises error if no codes are available or code already exists for this CoA.

        Duplicate code logic:
        - If two sibling accounts have the same name and one code is the other with a trailing '0',
          use only the one without the '0'. Otherwise, use both.
        """
        from ledger.io.utils.coa_code_map import get_available_account_codes


        available_accounts = get_available_account_codes()
        used_codes = set(self.__class__.objects.filter(coa_model=self.coa_model).values_list('code', flat=True))

        for acc in available_accounts:
            if acc["code"] not in used_codes:
                self.name = acc["name"]
                self.role = acc["role"]
                return acc["code"]
        raise AccountModelValidationError("No available account codes left in chart_of_accounts_with_roles.json for this Chart of Accounts.")

    # def clean(self):
    #     # If code is not set, assign from JSON
    #     if not self.code:
    #         self.code = self.generate_next_coa_code()
    #     # Uniqueness check
    #     if self.__class__.objects.filter(coa_model=self.coa_model, code=self.code).exclude(pk=self.pk).exists():
    #         raise AccountModelValidationError(f"Account code {self.code} already exists for this Chart of Accounts.")
    #     if LEDGER_ACCOUNT_CODE_USE_PREFIX:
    ...
    def clean(self):
        if not self.code and LEDGER_ACCOUNT_CODE_GENERATE:
    ...
    def get_absolute_url(self) -> str:
        return reverse(
            viewname='ledger:account-detail',
            kwargs={
    ...
    def get_update_url(self) -> str:
        return reverse(
            viewname='ledger:account-update',
            kwargs={
                'account_pk': self.uuid,
                'entity_slug': self.entity_slug,
                'coa_slug': self.coa_slug
    ...
    def get_action_deactivate_url(self) -> str:
        return reverse(
            viewname='ledger:account-action-deactivate',
            kwargs={
                'account_pk': self.uuid,
                'entity_slug': self.entity_slug,
                'coa_slug': self.coa_slug
    ...
    def get_action_activate_url(self) -> str:
        return reverse(
            viewname='ledger:account-action-activate',
            kwargs={
                'account_pk': self.uuid,
                'entity_slug': self.entity_slug,
                'coa_slug': self.coa_slug
    ...
    def get_action_lock_url(self) -> str:
        return reverse(
            viewname='ledger:account-action-lock',
            kwargs={
                'account_pk': self.uuid,
                'entity_slug': self.entity_slug,
                'coa_slug': self.coa_slug
    ...
    def get_action_unlock_url(self) -> str:
        return reverse(
            viewname='ledger:account-action-unlock',
            kwargs={
                'account_pk': self.uuid,
                'entity_slug': self.entity_slug,
                'coa_slug': self.coa_slug
