...
from ledger.models.mixins import CreateUpdateMixIn, MarkdownNotesMixIn
...
class ClosingEntryModelAbstract(CreateUpdateMixIn, MarkdownNotesMixIn):
    ...
    ledger_model = models.OneToOneField('ledger.LedgerModel', on_delete=models.CASCADE)
    ...
    def get_mark_as_posted_url(self, entity_slug: Optional[str] = None) -> str:
        ...
        return reverse(viewname='ledger:closing-entry-action-mark-as-posted',
                       kwargs={
    ...
    def get_mark_as_unposted_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(viewname='ledger:closing-entry-action-mark-as-unposted',
                       kwargs={
                           'entity_slug': entity_slug,
                           'closing_entry_pk': self.uuid
                       })
    ...
    def get_update_transactions_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(viewname='ledger:closing-entry-action-update-txs',
                       kwargs={
                           'entity_slug': entity_slug,
                           'closing_entry_pk': self.uuid
                       })
    ...
    def get_delete_url(self, entity_slug: Optional[str] = None) -> str:
        if not entity_slug:
            entity_slug = self.entity_model.slug
        return reverse(viewname='ledger:closing-entry-action-delete',
                       kwargs={
                           'entity_slug': entity_slug,
                           'closing_entry_pk': self.uuid
                       })
    ...
    def get_list_url(self):
        return reverse(
            viewname='ledger:closing-entry-list',
            kwargs={
                'entity_slug': self.entity_model.slug
            }
        )
...
class ClosingEntryTransactionModelAbstract(CreateUpdateMixIn):
    uuid = models.UUIDField(default=uuid4, editable=False, primary_key=True)
    closing_entry_model = models.ForeignKey('ledger.ClosingEntryModel',
                                            on_delete=models.CASCADE)
    account_model = models.ForeignKey('ledger.AccountModel',
                                      on_delete=models.RESTRICT,
                                      verbose_name=_('Account Model'))
    unit_model = models.ForeignKey('ledger.EntityUnitModel',
                                   null=True,
                                   blank=True,
                                   on_delete=models.RESTRICT,
                                   verbose_name=_('Entity Model'))
