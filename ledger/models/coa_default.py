...
The Ledger Default Chart of Accounts must include the following fields:
    * Code - String
    * Role - A choice from any of the possible account roles (see ledger.roles module).
...
from ledger.exceptions import DjangoLedgerConfigurationError
from ledger.io import roles, ROOT_ASSETS, ROOT_INCOME, ROOT_EXPENSES, ROOT_LIABILITIES, ROOT_CAPITAL, ROOT_COGS
from ledger.settings import LEDGER_DEFAULT_COA
...
def get_default_coa() -> List[Dict]:
    if LEDGER_DEFAULT_COA is not None and isinstance(LEDGER_DEFAULT_COA, list):
        return LEDGER_DEFAULT_COA
    return DEFAULT_CHART_OF_ACCOUNTS


if LEDGER_DEFAULT_COA:
    LEDGER_DEFAULT_COA = get_default_coa()
