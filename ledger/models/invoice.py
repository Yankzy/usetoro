...
which the :func:`EntityModel <ledger.models.entity.EntityModel>` issues to its customers for the supply of
goods or services. The model manages all the Sales Invoices which are issued by the :func:`EntityModel
<ledger.models.entity.EntityModel>`. In addition to tracking the invoice amount , it tracks the receipt and
due amount.
...
from ledger.io import ASSET_CA_CASH, ASSET_CA_RECEIVABLES, LIABILITY_CL_DEFERRED_REVENUE
from ledger.io.io_core import get_localtime, get_localdate
...
from ledger.settings import LEDGER_DOCUMENT_NUMBER_PADDING, LEDGER_INVOICE_NUMBER_PREFIX
...
class InvoiceModelAbstract(
    ...
    The InvoiceModel inherits functionality from the following MixIns:
        ...
        1. :func:`LedgerWrapperMixIn <ledger.models.mixins.LedgerWrapperMixIn>`
        2. :func:`PaymentTermsMixIn <ledger.models.mixins.PaymentTermsMixIn>`
        3. :func:`MarkdownNotesMixIn <ledger.models.mixins.MarkdownNotesMixIn>`
    ...
    invoice_number: str
        ...
        Prefix be customized with LEDGER_INVOICE_NUMBER_PREFIX setting.
    ...
    customer = models.ForeignKey('ledger.CustomerModel',
                                 on_delete=models.RESTRICT,
                                 verbose_name=_('Customer'))

    cash_account = models.ForeignKey('ledger.AccountModel',
                                     on_delete=models.RESTRICT,
                                     verbose_name=_('Cash Account'),
                                     related_name=f'{REL_NAME_PREFIX}_cash_account')
    prepaid_account = models.ForeignKey('ledger.AccountModel',
                                        on_delete=models.RESTRICT,
                                        verbose_name=_('Prepaid Account'),
                                        related_name=f'{REL_NAME_PREFIX}_prepaid_account')
    unearned_account = models.ForeignKey('ledger.AccountModel',
                                         on_delete=models.RESTRICT,
    ...
    invoice_items = models.ManyToManyField('ledger.ItemModel',
                                           through='ledger.ItemTransactionModel',
                                           through_fields=('invoice_model', 'item_model'),
                                           verbose_name=_('Invoice Items'))

    ce_model = models.ForeignKey('ledger.EstimateModel',
                                 on_delete=models.RESTRICT,
                                 null=True,
                                 blank=True,
    ...
    def get_mark_as_draft_url(self):
        ...
        str
            HTML ID as a String
        """
        return reverse('ledger:invoice-action-mark-as-draft',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
    ...
    def get_mark_as_review_url(self):
        ...
        str
            InvoiceModel mark-as-review action URL.
        """
        return reverse('ledger:invoice-action-mark-as-review',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_approved_url(self):
        ...
        str
            InvoiceModel mark-as-approved action URL.
        """
        return reverse('ledger:invoice-action-mark-as-approved',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_paid_url(self, entity_slug: Optional[str] = None):
        ...
        if not entity_slug:
            entity_slug = self.ledger.entity.slug
        return reverse('ledger:invoice-action-mark-as-paid',
                       kwargs={
                           'entity_slug': entity_slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_void_url(self):
        ...
        _______
            InvoiceModel mark-as-void action URL.
        """
        return reverse('ledger:invoice-action-mark-as-void',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    def get_mark_as_canceled_url(self):
        ...
        str
            InvoiceModel mark-as-canceled action URL.
        """
        return reverse('ledger:invoice-action-mark-as-canceled',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
    ...
    #     if not entity_slug:
    #         entity_slug = self.ledger.entity.slug
    #     return reverse('ledger:invoice-action-mark-as-delete',
    #                    kwargs={
    #                        'entity_slug': entity_slug,
    #                        'invoice_pk': self.uuid
    #                    })
    ...
    def generate_invoice_number(self, commit: bool = False) -> str:
        ...
        if self.can_generate_invoice_number():
            with transaction.atomic(durable=True):
                ...
                seq = str(state_model.sequence).zfill(LEDGER_DOCUMENT_NUMBER_PADDING)
                self.invoice_number = f'{LEDGER_INVOICE_NUMBER_PREFIX}-{state_model.fiscal_year}-{seq}'
    ...
    def get_absolute_url(self):
        return reverse('ledger:invoice-detail',
                       kwargs={
                           'entity_slug': self.ledger.entity.slug,
                           'invoice_pk': self.uuid
                       })
