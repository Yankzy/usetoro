...
from ledger.settings import (LEDGER_TRANSACTION_MAX_TOLERANCE, LEDGER_DOCUMENT_NUMBER_PADDING,
                                    LEDGER_EXPENSE_NUMBER_PREFIX, LEDGER_INVENTORY_NUMBER_PREFIX,
                                    LEDGER_PRODUCT_NUMBER_PREFIX)
...
class ItemModelAbstract(CreateUpdateMixIn):
    ...
    uom = models.ForeignKey('ledger.UnitOfMeasureModel',
    ...
    def get_item_number_prefix(self):
        if self.is_expense():
            return LEDGER_EXPENSE_NUMBER_PREFIX
        elif self.is_inventory():
            return LEDGER_INVENTORY_NUMBER_PREFIX
        elif self.is_product() or self.is_service():
            return LEDGER_PRODUCT_NUMBER_PREFIX
...
class ItemTransactionModelAbstract(CreateUpdateMixIn):
    ...
    item_model = models.ForeignKey('ledger.ItemModel',
                                   on_delete=models.RESTRICT,
                                   verbose_name=_('Item Model'))
    bill_model = models.ForeignKey('ledger.BillModel',
                                   on_delete=models.RESTRICT,
                                   null=True,
                                   blank=True,
                                   verbose_name=_('Bill Model'))
    invoice_model = models.ForeignKey('ledger.InvoiceModel',
                                      on_delete=models.RESTRICT,
                                      null=True,
                                      blank=True,
    ...
    po_model = models.ForeignKey('ledger.PurchaseOrderModel',
                                 on_delete=models.RESTRICT,
                                 null=True,
                                 blank=True,
                ...
                if self.po_total_amount is not None and self.total_amount > self.po_total_amount:
                    ...
                    if diff > LEDGER_TRANSACTION_MAX_TOLERANCE:
                        raise ValidationError(
                            ...
                            f'exceeds tolerance of {LEDGER_TRANSACTION_MAX_TOLERANCE}')
