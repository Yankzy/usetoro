"""
The LedgerModel is the heart of Ledger. It is a self-contained unit of accounting that implements a
...
(see ledger.models.mixins.LedgerWrapperMixIn), which is the foundation of LedgerModel abstractions such as the
...
Also, the LedgerModel inherits functionality from the all mighty IOMixIn (see ledger.io.io_mixin.IOMixIn),
...
The Ledger core model follows the following structure:
...
from ledger.io.io_core import IOMixIn
from ledger.models import lazy_loader
...
class LedgerModelAbstract(CreateUpdateMixIn, IOMixIn):
    ...
    def post_journal_entries(self, commit: bool = True, **kwargs):
        ...
        for je_model in je_model_qs:
            ...
            modified.append(je_model)

        JournalEntryModel = self.journal_entries.model

        if commit and modified:
            # Persist the in-memory changes (use the model manager bulk_update with the modified instances)
            JournalEntryModel.objects.bulk_update(objs=modified, fields=['posted', 'locked', 'activity', 'updated'])
            # return a queryset representing the persisted objects for compatibility with callers
            return JournalEntryModel.objects.filter(pk__in=[m.pk for m in modified])
        # if not committing, return the list of modified instances
        return modified
    ...
    def get_absolute_url(self) -> str:
        ...
        str
            URL as a string.
        """
        return reverse(viewname='ledger:je-list',
                       kwargs={
    ...
    def get_create_url(self) -> str:
        ...
        str
            URL as a string.
        """
        return reverse('ledger:ledger-create',
                       kwargs={
                           'entity_slug': self.entity.slug
                       })
    ...
    def get_update_url(self) -> str:
        ...
        str
            URL as a string.
        """
        return reverse('ledger:ledger-update',
                       kwargs={
                           'entity_slug': self.entity.slug,
                           'ledger_pk': self.uuid
                       })
    ...
    def get_journal_entry_create_url(self) -> str:
        return reverse('ledger:je-create',
                       kwargs={
                           'entity_slug': self.entity_slug,
                           'ledger_pk': self.uuid
                       })
    ...
    def get_action_post_journal_entries_url(self):
        return reverse(
            viewname='ledger:ledger-action-post-journal-entries',
            kwargs={
                'entity_slug': self.entity_slug,
                'ledger_pk': self.uuid
            }
    ...
    def get_action_lock_journal_entries_url(self):
        return reverse(
            viewname='ledger:ledger-action-lock-journal-entries',
            kwargs={
                'entity_slug': self.entity_slug,
                'ledger_pk': self.uuid
            }
