...
class LazyLoader:
    """
    A class that provides lazy-loading functionality for various models in the ledger app.

    Attributes:
        app_config (AppConfig): The AppConfig object for the ledger app.
    ...
    app_config = apps.get_app_config(app_label='ledger')
    ...
    def get_balance_sheet_report_class(self):
        if not self.BALANCE_SHEET_REPORT_CLASS:
            from ledger.report.balance_sheet import BalanceSheetReport
    ...
    def get_income_statement_report_class(self):
        if not self.INCOME_STATEMENT_REPORT_CLASS:
            from ledger.report.income_statement import IncomeStatementReport
    ...
    def get_cash_flow_statement_report_class(self):
        if not self.CASH_FLOW_STATEMENT_REPORT_CLASS:
            from ledger.report.cash_flow_statement import CashFlowStatementReport
