...
from ledger.io.roles import ASSET_CA_CASH, ASSET_CA_PREPAID, LIABILITY_CL_DEFERRED_REVENUE, \
    LIABILITY_CL_ACC_PAYABLE
from ledger.models import EntityModel, BillModel, VendorModel
from ledger.tests.base import DjangoLedgerBaseTest
from ledger.urls.bill import urlpatterns as bill_urls
...
class BillModelTests(DjangoLedgerBaseTest):
    ...
    def test_protected_views(self):
        ...
        for path, kwargs in self.URL_PATTERNS.items():
            ...
            url = reverse(f'ledger:{path}', kwargs=url_kwargs)
            response = self.CLIENT.get(url, follow=False)
            redirect_url = urlparse(response.url)
            redirect_path = redirect_url.path
            login_path = reverse(viewname='ledger:login')
    ...
    def test_bill_list(self):
        ...
        bill_list_url = reverse('ledger:bill-list',
                                kwargs={
        ...
        for bill_model in bill_model_qs:
            bill_detail_url = reverse('ledger:bill-detail',
                                      kwargs={
                                          'entity_slug': entity_model.slug,
                                          'bill_pk': bill_model.uuid
                                      })
            bill_update_url = reverse('ledger:bill-update',
                                      kwargs={
                                          'entity_slug': entity_model.slug,
                                          'bill_pk': bill_model.uuid
                                      })
            # bill_delete_url = reverse('ledger:bill-delete',
            #                           kwargs={
            #                               'entity_slug': entity_model.slug,
            #                               'bill_pk': bill_model.uuid
            #                           })
            # mark_as_paid_url = reverse('ledger:bill-mark-paid',
            #                            kwargs={
            #                                'entity_slug': entity_model.slug,
            #                                'bill_pk': bill_model.uuid
            #                            })
    ...
    def test_bill_create(self):
        ...
        bill_create_url = reverse('ledger:bill-create',
                                  kwargs={
                                      'entity_slug': entity_model.slug
                                  })
        ...
        # bill_data['prepaid_account'] = cash_account.uuid
        # create_response = self.CLIENT.post(bill_create_url, data=bill_data, follow=True)
        # self.assertTrue(create_response.resolver_match.view_name, 'ledger:bill-detail')
