...
from ledger.models import EntityModel, PurchaseOrderModel
from ledger.tests.base import DjangoLedgerBaseTest
from ledger.urls.purchase_order import urlpatterns as po_urls
...
class PurchaseOrderModelTests(DjangoLedgerBaseTest):
    ...
    def test_purchase_order_list(self):
        ...
        self.login_client()
        entity_model: EntityModel = choice(self.ENTITY_MODEL_QUERYSET)
        po_list_url = reverse('ledger:po-list', kwargs={'entity_slug': entity_model.slug})
        ...
        for po_model in po_model_qs:
            po_detail_url = reverse('ledger:po-detail',
                                    kwargs={
                                        'entity_slug': entity_model.slug,
                                        'po_pk': po_model.uuid
                                    })

            po_delete_url = reverse('ledger:po-delete',
                                    kwargs={
                                        'entity_slug': entity_model.slug,
                                        'po_pk': po_model.uuid
                                    })
    ...
    def test_purchase_order_create(self):
        ...
        po_create_url = reverse('ledger:po-create', kwargs={'entity_slug': entity_model.slug})
        ...
        list_path = reverse('ledger:po-list', kwargs={'entity_slug': entity_model.slug})
    ...
    def test_purchase_order_detail(self):
        ...
        for po_model in po_model_qs:
            po_detail_url = reverse('ledger:po-detail', kwargs={
                'entity_slug': entity_model.slug,
                'po_pk': po_model.uuid
            })
            po_update_url = reverse('ledger:po-update', kwargs={
                'entity_slug': entity_model.slug,
                'po_pk': po_model.uuid
            })
            po_delete_url = reverse('ledger:po-delete', kwargs={
                'entity_slug': entity_model.slug,
                'po_pk': po_model.uuid
            })
