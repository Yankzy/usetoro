...
from ledger.models import EntityModel, PurchaseOrderModel, EstimateModel, BillModelQuerySet
...
class BillModelCreateView(BillModelModelBaseView, CreateView):
    template_name = 'ledger/bills/bill_create.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.for_purchase_order:
            ...
            form_action = reverse('ledger:bill-create-po',
                                  kwargs={
                                      'entity_slug': self.kwargs['entity_slug'],
                                      'po_pk': po_model.uuid
            ...
            form_action = reverse('ledger:bill-create-estimate',
                                  kwargs={
                                      'entity_slug': self.kwargs['entity_slug'],
                                      'ce_pk': estimate_model.uuid
                                  })
        else:
            form_action = reverse('ledger:bill-create',
                                  kwargs={
                                      'entity_slug': self.kwargs['entity_slug'],
                                  })
...
class BillModelDetailView(BillModelModelBaseView, DetailView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if not bill_model.is_configured():
            link = format_html(f"""
            <a href="{reverse("ledger:bill-update", kwargs={
                'entity_slug': self.kwargs['entity_slug'],
                'bill_pk': bill_model.uuid
            })}">here</a>
            """)
...
class BillModelUpdateView(BillModelModelBaseView, UpdateView):
    ...
    template_name = 'ledger/bills/bill_update.html'
    extra_context = {
        'header_subtitle_icon': 'uil:bill'
    }
    ...
    def get(self, request, *args, **kwargs):
        if self.action_update_items:
            return HttpResponseRedirect(
                redirect_to=reverse('ledger:bill-update',
                                    kwargs={
                                        'entity_slug': self.kwargs['entity_slug'],
                                        'bill_pk': self.kwargs['bill_pk']
                                    })
...
class BaseBillActionView(BillModelModelBaseView, RedirectView, SingleObjectMixin):
    ...
    def get_redirect_url(self, *args, **kwargs):
        return reverse('ledger:bill-update',
                       kwargs={
