...
You are not responsible for exact global allocation arithmetic or searching for amount matches. We have provided a list of PRE-CALCULATED PLAUSIBLE CANDIDATES. Your job is to evaluate these candidates against the EVIDENCE and generate explicit reconciliation hypotheses for the `global_reconciliation_optimizer` tool.
...
3. Unresolved is a valid outcome. Do NOT create hypotheses for unresolved bank movements; simply omit them from your optimizer requests.
...
6. CRITICAL: If the pre-calculated list shows multiple overlapping candidates for the same bank movement, you MUST generate a SEPARATE hypothesis for EACH plausible match in your tool call. Assign them appropriate utilities based on evidence, and let the optimizer mathematically resolve the overlap.

--- EXAMPLES OF EXACT TOOL CALL STRUCTURES ---

EXAMPLE 1: SIMPLE (Clear 1-to-1 Match)
Evidence: Bank B1 (10,000 MAD) explicitly references INV-01 (A1).
Tool `hypotheses` array:
[
  {
    "id": "H1",
    "utility": 950,
...
Tool `hypotheses` array (Note the overlapping proposals for B3):
[
  {
    "id": "H3_option1",
    "utility": 600,
...
- Call the optimizer.
- Review conflicts/diagnostics and revise if necessary.
- When stable, emit the final ProposedState JSON.
"""