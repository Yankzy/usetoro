...
// ExecDocumentURL passes a presigned HTTPS URL directly to OpenAI for visual document analysis.
func (r *Runtime) ExecDocumentURL(ctx context.Context, prompt string, systemPrompt string, fileURL string, fileName string) (string, error) {
	...
	modelToCall := r.effectiveModel(ctx)
	r.Logger.Info("Executing visual LLM document extraction via URL", "model", modelToCall, "file_name", fileName, "url", fileURL)

	fullUserText := prompt
	if systemPrompt != "" {
		fullUserText = fmt.Sprintf("SYSTEM INSTRUCTIONS:\n%s\n\nUSER PROMPT:\n%s", systemPrompt, prompt)
	}

	switch pc.Paradigm {
	case ParadigmResponses:
		inputItems := responses.ResponseInputParam{
			responses.ResponseInputItemParamOfMessage(fullUserText, responses.EasyInputMessageRoleUser),
			responses.ResponseInputItemParamOfMessage(
				responses.ResponseInputMessageContentListParam{
					{
						OfInputFile: &responses.ResponseInputFileParam{
							Type:    constant.InputFile("input_file"),
							FileURL: openai.String(fileURL),
						},
					},
				},
		...
		if err != nil {
			return "", fmt.Errorf("failed visual document extraction via URL (responses API): %w", err)
		}
		r.emitTelemetry(ctx, pc, modelToCall, resp.Usage.InputTokens, resp.Usage.OutputTokens, resp.Usage.TotalTokens)
		return resp.OutputText(), nil

	default:
		return "", fmt.Errorf("document extraction via URL requires a model supporting the Responses API (e.g. gpt-4.5-preview); Chat Completions API does not support native document parsing")
	}
}
